...
- 🟡 На перспективу:
  ...
  - ✅ Экспорт CSV с учетом фильтров и локализации
...
- ✅ **Import**: импорт CSV файлов с сопоставлением колонок и категорий
- ✅ **Export**: экспорт транзакций в CSV с учетом фильтров и локализации
...
- ✅ **Import**: импорт CSV файлов с настройкой маппинга колонок и категорий
- ✅ **Export**: экспорт транзакций в CSV с поддержкой фильтров и локализации
...
- ✅ **Загрузка файлов**: поддержка CSV файлов с автоматическим определением кодировки
- ✅ **Настройка маппинга**: интуитивный интерфейс для сопоставления колонок CSV с полями транзакций
- ✅ **Предварительный просмотр**: отображение первых строк данных для проверки корректности маппинга
- ✅ **Категоризация**: автоматическое сопоставление категорий или ручное назначение
- ✅ **Валидация**: проверка данных перед импортом с отображением ошибок
- ✅ **Пакетная обработка**: импорт больших файлов с прогресс-баром
- ✅ **Обработка ошибок**: детальная информация о проблемах в данных

### 📤 Экспорт CSV

- ✅ **Экспорт с фильтрами**: экспорт транзакций с учетом всех примененных фильтров (тип, даты, поиск, категории)
- ✅ **Локализация**: автоматическое определение языка интерфейса и экспорт заголовков на соответствующем языке
- ✅ **Формат совместимости**: CSV файл в том же формате, что ожидается при импорте
- ✅ **Названия категорий**: экспорт названий категорий на выбранном языке
- ✅ **Автоматическое скачивание**: файл скачивается с именем `transactions_YYYY-MM-DD.csv`
- ✅ **Модальное окно подтверждения**: показывает количество транзакций для экспорта
- ✅ **Обработка ошибок**: корректная обработка BigInt значений и других типов данных
