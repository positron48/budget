...
### Вариант 1: Docker (для разработки)
```bash
...
### Вариант 2: Деплой через артефакты (для продакшена)
```bash
# Клонирование репозитория
git clone https://github.com/positron48/budget
cd budget
...
sudo apt update
sudo apt install -y curl jq nginx postgresql redis-server

# Деплой последней версии
sudo make deploy-all-artifact TAG=latest
```

📖 **Подробная документация**: [QUICK_DEPLOY.md](QUICK_DEPLOY.md) | [ARTIFACT_DEPLOYMENT.md](doc/ARTIFACT_DEPLOYMENT.md)
...
### Деплой через артефакты
| Команда | Описание |
|---------|----------|
| `make check-updates` | Проверка доступных обновлений |
| `make deploy-all-artifact TAG=latest` | Полный деплой последней версии |
| `make deploy-backend-artifact TAG=v1.0.0` | Деплой бэкенда конкретной версии |
| `make deploy-frontend-artifact TAG=v1.0.0` | Деплой фронтенда конкретной версии |
