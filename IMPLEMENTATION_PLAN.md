...
- ✅ CI — сделано: добавлен GitHub Actions (buf lint/generate, golangci-lint, go test с порогом покрытия ≥80%), кэш Buf/Go и публикация артефакта coverage.
- ✅ Observability — сделано: Logging/Recovery, MetricsUnaryInterceptor и /metrics (Prometheus), otelgrpc interceptor и базовая OTLP/HTTP инициализация; добавлены compose‑сервисы Prometheus/Grafana/OTEL Collector и пример дашборда. (Трассировка pgx — отложено как улучшение.)
...
Статус: ✅ сделано — unit‑тесты есть (argon2, jwt issuer, интерсептор auth), интеграционные сценарии in‑process завершены (Auth/Tenant/Category/Transaction/Report/Fx).
...
Статус: ✅ сделано — реализованы все серверы; unit‑тесты и интеграционные сценарии (bufconn) покрывают основные пути.
