...
type repoStub struct{ u domain.User }
func (r repoStub) GetByID(ctx context.Context, id string) (domain.User, error) { return domain.User{ID: id, Email: "e@x", Name: "N"}, nil }
func (r repoStub) UpdateProfile(ctx context.Context, id, name, locale string) (domain.User, error) { return domain.User{ID: id, Email: "e@x", Name: name, Locale: locale}, nil }
...
func TestService_GetMe_UpdateProfile(t *testing.T) {
    ...
    if _, err := svc.GetMe(context.Background(), "u1"); err != nil { t.Fatalf("get: %v", err) }
    u, err := svc.UpdateProfile(context.Background(), "u1", "New", "ru")
    if err != nil || u.Name != "New" || u.Locale != "ru" { t.Fatalf("update: %v %#v", err, u) }
}
