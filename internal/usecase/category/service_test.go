...
func (s *stubRepo) Get(ctx context.Context, id string) (domain.Category, error) {
	s.got = domain.Category{ID: id}
	return s.got, nil
}
...
func TestService_CRUD(t *testing.T) {
	...
	if !reflect.DeepEqual(c.Translations, tr) {
		t.Fatalf("translations mismatch")
	}

	up, err := svc.Update(ctx, "c1", "food2", nil, false, tr)
	if err != nil || up.Code != "food2" || up.IsActive {
		t.Fatalf("update: %v %#v", err, up)
	}

	if err := svc.Delete(ctx, "c1"); err != nil || r.delID != "c1" {
	...
	if err != nil || len(lst) != 1 || lst[0].TenantID != "t1" {
