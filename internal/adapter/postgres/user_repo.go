...
func (r *UserRepo) CreateWithDefaultTenant(ctx context.Context, email, passwordHash, name, locale, tenantName string) (User, Tenant, error) {
    ...
    if err != nil { return User{}, Tenant{}, err }
    defer func() { _ = tx.Rollback(ctx) }()

    email = strings.ToLower(strings.TrimSpace(email))
    if tenantName == "" { tenantName = "My Budget" }
    ...
    if err := tx.Commit(ctx); err != nil { return User{}, Tenant{}, err }
    return u, t, nil
}
...
func (r *UserRepo) GetByEmail(ctx context.Context, email string) (User, []TenantMembership, error) {
    ...
    if err != nil {
        if errors.Is(err, pgx.ErrNoRows) { return User{}, nil, err }
        return User{}, nil, err
    }
    ...
    for rows.Next() {
        ...
        if err := rows.Scan(&m.TenantID, &m.Role, &m.IsDefault); err != nil { return User{}, nil, err }
        ms = append(ms, m)
    }
