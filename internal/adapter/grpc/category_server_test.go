...
func (s *catStubRepo) List(ctx context.Context, tenantID string, kind domain.CategoryKind, includeInactive bool) ([]domain.Category, error) {
	if s.list == nil {
		s.list = []domain.Category{{ID: "c1", TenantID: tenantID, Kind: kind, Translations: []domain.CategoryTranslation{{Locale: "en", Name: "Food"}, {Locale: "ru", Name: "Еда"}}}}
	}
	return s.list, nil
}
...
func TestCategoryServer_CRUD_And_Locale(t *testing.T) {
	...
	if err != nil || out.GetCategory().GetId() == "" {
	...
	if err != nil || up.GetCategory().GetCode() != "food2" || up.GetCategory().GetIsActive() {
	...
	if err != nil || len(g.GetCategory().GetTranslations()) == 0 || g.GetCategory().GetTranslations()[0].GetLocale() != "ru" {
		t.Fatalf("get locale: %v %#v", err, g)
	}

	// list with locale
	lst, err := srv.ListCategories(ctx, &budgetv1.ListCategoriesRequest{Kind: budgetv1.CategoryKind_CATEGORY_KIND_EXPENSE, Locale: "en"})
	if err != nil || len(lst.GetCategories()) != 1 || lst.GetCategories()[0].GetTranslations()[0].GetLocale() != "en" {
		t.Fatalf("list: %v %#v", err, lst)
	}

	// delete
	if _, err := srv.DeleteCategory(ctx, &budgetv1.DeleteCategoryRequest{Id: out.GetCategory().GetId()}); err != nil {
...
func (errCatRepo) Create(ctx context.Context, tenantID string, kind domain.CategoryKind, code string, parentID *string, isActive bool, translations []domain.CategoryTranslation) (domain.Category, error) {
	return domain.Category{}, errors.New("boom")
}

func (errCatRepo) Update(ctx context.Context, id string, code string, parentID *string, isActive bool, translations []domain.CategoryTranslation) (domain.Category, error) {
	return domain.Category{}, errors.New("boom")
}
func (errCatRepo) Delete(ctx context.Context, id string) error { return errors.New("boom") }
func (errCatRepo) Get(ctx context.Context, id string) (domain.Category, error) {
	return domain.Category{}, errors.New("boom")
}

func (errCatRepo) List(ctx context.Context, tenantID string, kind domain.CategoryKind, includeInactive bool) ([]domain.Category, error) {
...
func TestCategoryServer_MapError(t *testing.T) {
	...
	srv := NewCategoryServer(svc)
	ctx := ctxutil.WithTenantID(context.Background(), "t1")
	if _, err := srv.CreateCategory(ctx, &budgetv1.CreateCategoryRequest{Kind: budgetv1.CategoryKind_CATEGORY_KIND_EXPENSE, Code: "x"}); err == nil {
		t.Fatal("expected error")
	}
	if _, err := srv.UpdateCategory(ctx, &budgetv1.UpdateCategoryRequest{Id: "c1"}); err == nil {
		t.Fatal("expected error")
	}
	if _, err := srv.GetCategory(ctx, &budgetv1.GetCategoryRequest{Id: "c1"}); err == nil {
		t.Fatal("expected error")
	}
	if _, err := srv.ListCategories(ctx, &budgetv1.ListCategoriesRequest{Kind: budgetv1.CategoryKind_CATEGORY_KIND_EXPENSE}); err == nil {
		t.Fatal("expected error")
	}
	if _, err := srv.DeleteCategory(ctx, &budgetv1.DeleteCategoryRequest{Id: "c1"}); err == nil {
		t.Fatal("expected error")
	}
}
