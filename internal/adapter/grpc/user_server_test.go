...
func TestUserServer_GetAndUpdate(t *testing.T) {
    ...
    if me.GetUser().GetId() != "u1" { t.Fatalf("expected user id u1, got %q", me.GetUser().GetId()) }
...
func TestUserServer_ChangePassword(t *testing.T) {
    ...
    if _, err := srv.ChangePassword(ctx, &budgetv1.ChangePasswordRequest{CurrentPassword: "old", NewPassword: "new"}); err != nil { t.Fatalf("change: %v", err) }
}
...
func TestUserServer_ChangePassword_InvalidCurrent(t *testing.T) {
    ...
    srv := NewUserServer(svc, func(ctx context.Context, userID string) (string, error) { return "h", nil })
    if _, err := srv.ChangePassword(context.Background(), &budgetv1.ChangePasswordRequest{CurrentPassword: "bad", NewPassword: "new"}); err == nil { t.Fatal("expected error") }
}

func TestUserServer_UpdateProfile_Invalid(t *testing.T) {
    svc := useuser.NewService(userRepoStub{}, passHasherStub{})
    srv := NewUserServer(svc, func(ctx context.Context, userID string) (string, error) { return "h", nil })
    if _, err := srv.UpdateProfile(context.Background(), &budgetv1.UpdateProfileRequest{}); err == nil { t.Fatal("expected invalid argument") }
}
