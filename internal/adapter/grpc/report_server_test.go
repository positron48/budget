...
func TestReportServer_TenantFromContext(t *testing.T) {
    ...
    if _, err := srv.GetMonthlySummary(ctx, &budgetv1.GetMonthlySummaryRequest{Year: 2025, Month: 1}); err != nil { t.Fatalf("unexpected: %v", err) }
    if svc.got != "t1" { t.Fatalf("tenant id not passed: %q", svc.got) }
}
